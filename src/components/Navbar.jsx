...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="flex  items-center justify-between lg:justify-start px-4 lg:px-16 py-2 lg:py-4 text-sm ">
        ...
        <ul className="lg:flex items-center hidden  font-medium  justify-evenly w-full">
          {navbarLinks.map((link) => (
            <li key={link} className="hover:text-orange hover:underline">
              {link}
            </li>
          ))}
        </ul>
            ...
            <ul className="flex flex-col gap-4 font-medium  justify-evenly w-full">
              {navbarLinks.map((link) => (
                <li
                  key={link}
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="hover:text-orange hover:underline w-fit"
                >
                  {link}
                </li>
              ))}
